...
MA::MA(int _board_size, float _t_start, float _t_end, float _alpha)
    : board_size(_board_size), t_start(_t_start), t_end(_t_end),
      t_cur(_t_start), alpha(_alpha) {
...
void MA::run_threads() {
  ...
  for (unsigned int i{0}; i < num_threads; ++i) {
    vec.push_back(std::async(std::launch::async, &SA::run_return_only,
                             SA(board_size, t_start, t_end, alpha)));
  }
  ...
  std::cout << counter << " of " << num_threads
            << " threads returned an optimal Solution\n";
}
...
void MA::multithreaded_annealing() {
  ...
  std::cout << "Solving n-Queens Problem using a Multithreaded Simulated "
               "Annealing Algorithm\n-------\nEnter Board Size n=";
  std::cin >> board_size;
  ...
  std::cout << "Total Time consumed to find all solutions: " << time_consumed
            << " miliseconds\n";
