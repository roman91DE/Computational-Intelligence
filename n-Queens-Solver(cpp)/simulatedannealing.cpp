...
SA::SA(int _board_size, float _t_start, float _t_end, float _alpha)
    : board_size(_board_size), t_start(_t_start), t_end(_t_end),
      t_cur(_t_start), alpha(_alpha), cur_sol(Solution()), best_sol(),
      cur_fit(1000), best_fit(1000) {
...
void SA::run() {
  ...
  while ((t_cur > t_end) && (best_fit < 0)) {
    ...
    std::cout << "Temperature: " << t_cur << "\tCurrent Fit: " << cur_fit
              << "\tNew Fit: " << temp_fit << "\tBest Fit: " << best_fit
              << "\n";
...
void SA::simulated_annealing() {
  ...
  std::cout << "Solving n-Queens Problem using a Simulated Annealing "
               "Algorithm\n-------\nEnter Board Size n=";
  std::cin >> board_size;
