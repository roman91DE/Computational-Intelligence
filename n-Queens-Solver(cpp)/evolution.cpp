...
EA::EA(int _board, int _main, int _breading, int _tournament, int _seconds,
       int _elite, float _mr, float _cr)
    : board_dimension(_board), main_pop_size(_main),
      breading_pop_size(_breading), tournament_size(_tournament),
      time_limit(_seconds), m_rate(_mr), elite_size(_elite), c_rate(_cr),
      main_pop(Population(board_dimension, main_pop_size, m_rate, c_rate)),
      cur_generation(0) {
  main_pop.sort();
...
void EA::run(bool log) {
  ...
  if (log) {
    ...
    log_file << "Evolutionary Algorithm - " << board_dimension << " Queens\n"
             ...
             << cur_generation << "," << best_fit << "," << average_fit << "\n";
  }
  ...
  while ((time_consumed < time_limit) && (best_fit < 0)) {
    ...
    while (main_pop.get_size() < main_pop_size) {
      Solution child = Solution::onepoint_crossover(
          breading_pool.rand_select(), breading_pool.rand_select());
      child.swap_mutation();
    ...
    if (log) {
      log_file << cur_generation << "," << best_fit << "," << average_fit
               << "\n";
    }
  }
...
void EA::evolutionary_algorithm() {
  ...
  // init & run algorithm
  EA ea = EA(boardSize, mainPopSize, breadingPopSize, tournamentSize,
             time_limit, eliteSize, mutateRate, crossoverRate);
  ea.run(logResults);
}
