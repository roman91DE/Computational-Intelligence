...
from algorithms_HC_EA import calc_EA_tour, calc_HC_tour, calc_EA_tour_txt
from algorithms_insertion_heuristics import (
    calc_nearest_neighbor_tour,
    calc_in_order_tour,
...
def print_results_from_tsp_path(call_args, tsp_path):
    ...
    if call_args.need_HC:
        print("HC LENGTH:                {}".format(calc_HC_tour(tsp)))

    if call_args.need_EA:
        print("EA LENGTH:                {}".format(calc_EA_tour(tsp)))

    if call_args.need_EA_txt:
        print("EA AVERAGE LENGTH:        {}".format(calc_EA_tour_txt(tsp)))
