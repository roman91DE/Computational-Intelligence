...
def get_violated_busses(case):
    ...
    for i in range(0,len(case["bus"])):
        ...
        if (case["bus"][i][7] < case["bus"][i][12] or case["bus"][i][7] > case["bus"][i][11] ):
...
def hill_climber(case:dict, fitness_function:Callable, time_limit_secs:int, print_stats:bool=True):
    ...
    while (time.time() - t_start) < time_limit_secs:
        ...
        if print_stats:
            print("gen %5d fit %1.5f #viol %3d costs %1.5f %s" % (gen, fitness[0], fitness[1], fitness[2], extension_measures_to_str(case)))
...
def swap_mutation(network:Dict):
    ...
    for i in range(n):
        if random.random() < m_rate:
            ...
            while zv == i:
                zv = random.randint(0,n-1)
            network["extensions"][i][10], network["extensions"][zv][10] = network["extensions"][zv][10], network["extensions"][i][10]
...
def evolutionary_algorithm(case:Dict, time_limit_secs:int, population_size:int, mutation_func:Callable, crossover_func:Callable, fitness_func:Callable, elite_size:int=2, tournament_size:int=4, print_stats:bool=True) -> tuple:
    ...
    while (time.time() - t_start) < time_limit_secs:
        ...
        if print_stats:
            ...
            print("Generation: {:<3.0f}\tBest Solution: fitness={:<6.2f}\tviolations={:<3.0f}\tlength={:<6.2f}\n".format(cur_generation, temp[0], temp[1], temp[2]))
...
def simulated_annealing(case:Dict, time_limit_secs:int, fitness_function:Callable, mutation_function:Callable, t_start:float, t_end:float, print_stats:bool) -> tuple:
    ...
    while ((time.time() - t_start) < time_limit_secs) and (t > t_end):
        ...
        if print_stats:
            print("Temp: {:<10.2f}\tCurrent Fitness: {:<10.2f}\tBest Fitness: {:<10.2f}\tViolations (current): {:<10}\tViolations (best): {:<10}\n".format(t, current_fitness[0], best_fitness[0], current_fitness[1], best_fitness[1]))
...
def metropolis_algorithm(case:Dict, time_limit_secs:int, fitness_function:Callable, mutation_function:Callable, temperature:float, print_stats:bool) -> tuple:
    ...
    while ((time.time() - t_start) < time_limit_secs):
        ...
        if print_stats:
            print("Temp: {:<10.2f}\tCurrent Fitness: {:<10.2f}\tBest Fitness: {:<10.2f}\tViolations (current): {:<10}\tViolations (best): {:<10}\n".format(temperature, current_fitness[0], best_fitness[0], current_fitness[1], best_fitness[1]))
...
def simulated_annealing_population (case:Dict, time_limit_secs:int, population_size:int, fitness_function:Callable, mutation_function:Callable, t_start:float, t_end:float, print_stats:bool) -> tuple:
...
def metropolis_algorithm_population(case:Dict, time_limit_secs:int, population_size:int, fitness_function:Callable, mutation_function:Callable, temperature:float, print_stats:bool) -> tuple:
...
def run_hillclimber():
    ...
    return hill_climber(random_init(case), evaluate_objective_function, time_limit, printing)
...
def run_sa():
    ...
    case = case300extended2()
    return simulated_annealing(random_init(case),time_limit, evaluate_objective_function, flip_mutation, 1000000, 10, printing)

def run_sa_custom():

    case = case300extended2()
    return simulated_annealing(random_init(case),time_limit, evaluate_fitness, flip_mutation, 1000000, 10, printing)

def run_ea():

    case = case300extended2()
    return evolutionary_algorithm(case, time_limit, 25, flip_mutation, crossover_onepoint, evaluate_objective_function, print_stats=printing)

def run_ea_custom():

    case = case300extended2()
    return evolutionary_algorithm(case, time_limit, 25, flip_mutation, crossover_onepoint, evaluate_fitness, print_stats=printing)

def run_metropolis():

    case = case300extended2()
    return metropolis_algorithm(random_init(case),time_limit, evaluate_objective_function, flip_mutation, 2000, printing)

def run_metropolis_custom():

    case = case300extended2()
    return metropolis_algorithm(random_init(case),time_limit, evaluate_fitness, flip_mutation, 2000, printing)
...
def run_all_algorithms():
    ...
    if os.stat("results.tsv").st_size == 0:
        results.write("algorithm\tfitness(basic)\tfitness(custom)\tnum_violations\tlength_extensions\textension_pattern\n")

    algorithms = (run_hillclimber, run_hillclimber_custom, run_ea, run_ea_custom, run_sa, run_sa_custom, run_metropolis, run_metropolis_custom)
    ...
    for algorithm in algorithms:
        ...
        for i in range(n_runs):
            ...
            results.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(algorithm.__name__, evaluate_objective_function(return_tuple[1])[0], evaluate_fitness(return_tuple[1])[0], return_tuple[0][1], return_tuple[0][2], extension_measures_to_str(return_tuple[1])))
...
def metropolis_maxIterations(case:Dict, temperature:float=2000.0, max_iterations:int=10000) -> tuple:
...
# simulated annealing
def simulated_annealing_maxIterations(case:Dict, t_start:float=1000000, t_end:float=1, max_iterations:int=10000) -> tuple:
...
def contest():
    ...
    algorithms = (hc_maxIterations, metropolis_maxIterations, simulated_annealing_maxIterations)
    ...
    for algorithm in algorithms:
        ...
        for _ in range(rounds):
            ...
            log_file.write("{}\t{}\t{}\t{}\n".format(algorithm.__name__, ret[0][0], ret[0][1], ret[0][2]))
